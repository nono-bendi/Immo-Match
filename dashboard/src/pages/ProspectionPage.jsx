...
const STATUTS = [
  { value: 'visite',  label: 'Visité',       color: '#1d4ed8', bg: '#eff6ff' },
  { value: 'relance', label: 'À relancer',   color: '#b45309', bg: '#fffbeb' },
  { value: 'rdv',     label: 'RDV prévu',    color: '#6d28d9', bg: '#f5f3ff' },
  { value: 'devis',   label: 'Devis envoyé', color: '#0369a1', bg: '#f0f9ff' },
  { value: 'signe',   label: 'Signé',        color: '#065f46', bg: '#ecfdf5' },
  { value: 'refus',   label: 'Refus',        color: '#991b1b', bg: '#fef2f2' },
]
...
export default function ProspectionPage() {
  ...
  const relancesUrgentes = leads.filter(l => l.date_relance && l.date_relance <= today && l.statut !== 'signe' && l.statut !== 'refus')

  const stats = {
    total: leads.length,
    aRappeler: leads.filter(l => l.statut === 'relance').length,
    demo: leads.filter(l => l.statut === 'rdv').length,
  ...
  return (
    <div className="p-4 sm:p-6 max-w-5xl mx-auto">
      ...
      {relancesUrgentes.length > 0 && (
        <div className="mb-5 p-4 rounded-2xl flex items-center gap-3" style={{ background: '#fffbeb', border: '1px solid #fde68a' }}>
          ...
          <p className="text-sm font-medium" style={{ color: '#92400e' }}>
            ...
            {relancesUrgentes.map(l => l.nom).join(', ')}
          </p>
        </div>
      )}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
        ...
        <StatCard label="RDV prévus" value={stats.demo} color="#8b5cf6" icon={Calendar} />
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {filtered.map(lead => {
            const overdue = lead.date_relance && lead.date_relance <= today && lead.statut !== 'signe' && lead.statut !== 'refus'
            return (
              <div key={lead.id}
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      <h3 className="font-bold text-gray-800 truncate">{lead.nom}</h3>
                      <StatutBadge value={lead.statut} />
                    </div>

                    <div className="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-xs text-gray-400">
                      {lead.ville && (
                        <span className="flex items-center gap-1"><MapPin size={11} />{lead.ville}</span>
                      )}
                      {lead.contact && (
                        <span className="flex items-center gap-1"><UserCircle size={11} />{lead.contact}</span>
                      )}
                      {lead.telephone && (
      ...
      {deleteConfirm && (
        <div className="fixed inset-0 lg:left-64 bg-black/50 z-50 flex items-center justify-center p-4"
          ...
          <div className="bg-white rounded-2xl p-6 w-full max-w-sm shadow-2xl" onClick={e => e.stopPropagation()}>
            ...
            <p className="text-sm text-gray-500 mb-5">{deleteConfirm.nom}</p>
