...
def create_lead(data: dict, current_user: dict = Depends(get_current_user)):
    ...
    cursor.execute('''
        INSERT INTO prospection
            (nom_agence, adresse, nom_contact, telephone, email, statut, date_visite, date_relance, notes, created_at)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    ''', (
        data.get("nom_agence", ""),
        data.get("adresse"),
        data.get("nom_contact"),
        data.get("telephone"),
        data.get("email"),
        data.get("statut", "à visiter"),
...
def update_lead(lead_id: int, data: dict, current_user: dict = Depends(get_current_user)):
    ...
    allowed = {"nom_agence", "adresse", "nom_contact", "telephone", "email", "statut", "date_visite", "date_relance", "notes"}
