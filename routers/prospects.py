...
def add_prospect(prospect: dict, current_user: dict = Depends(get_current_user)):
    ...
    cursor.execute('''
        INSERT INTO prospects (date, nom, prenom, titre, mail, email2, telephone, telephone2, domicile, bien, villes, quartiers, budget_min, budget_max, criteres, etat, expo, stationnement, copro, exterieur, etage, destination, observation, chambre_plain_pied, plain_pied_total, sdb_min, wc_min)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    ''', (
...
def update_prospect(prospect_id: int, prospect: dict, current_user: dict = Depends(get_current_user)):
    ...
    conn.execute('''
        UPDATE prospects SET
            ...
            domicile = ?, bien = ?, villes = ?, quartiers = ?, budget_min = ?, budget_max = ?, criteres = ?,
